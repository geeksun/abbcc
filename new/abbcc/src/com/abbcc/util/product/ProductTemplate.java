 ...
 * 动态生成页面模板
 * @author dongyili
 * 创建产品时，选择下拉框和复选框时。格式：类别:1@hehe#3@heihei#4@lala
 * 选择文本时，可以添加html  如：<b>文本</b>
 */
public class ProductTemplate {
	...
	public String getTableStyle(Product product, String servletPath) {
		...
		}else if(product.getProductType()==Product.PRODUCT_TYPE_NO_PRODUCT){
			StringBuilder javascriptBuilder = new StringBuilder();
			javascriptBuilder.append("<script language=\"JavaScript\" type=\"text/javascript\">\n");
			javascriptBuilder.append("function checkProduct(){\n");
			javascriptBuilder.append(" return true;\n}\n</script>\n");
				...
				if (!isProperty) {
					...
					for (int t = 0; t < temp2.length; t++) {
						...
						for(int k=0;k<values.length;k++){
							...
							if(ss[0].equals(sv)){
								checked=true;
								tempBuilder.append("<input name ='"+_formName+"' value='"+ss[0]+"' type='checkbox' checked />"+ss[1]+"\n");
								break;
							} 
