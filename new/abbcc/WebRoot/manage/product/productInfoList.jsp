...
<%
	...
	boolean isOverdue=false;
	 if(overdue.equals(AppConstants.CPGOXX_OVERDUE)){
	    ...
	    Long inAuditCount=(Long)request.getAttribute("inAuditCount");
		Long inNetCount=(Long)request.getAttribute("inNetCount");
		Long unAuditCount=(Long)request.getAttribute("unAuditCount");
		Long overCount=(Long)request.getAttribute("overCount");
...
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3c.org/TR/1999/REC-html401-19991224/loose.dtd">
 <HTML>
	<HEAD>
		...
		<LINK rev=stylesheet href="<%=path%>/css/myali_search_v02.css"
		...
		<script type="text/javascript">
			...
			var action=document.<%=pagination!=null?pagination.getFormName():""%>.action="<%=path%>/manage/product.do?method=updateProductInfoAuditType&productInfoIds="+productInfoId+"&newAuditType="+auditType;
			document.<%=pagination!=null?pagination.getFormName():""%>.submit();
			}
			
			function deleteProduct(productInfoId){ 
			var page_form='<%=pagination!=null?pagination.getFormName():""%>'; 
			var action=document.<%=pagination!=null?pagination.getFormName():""%>.action="<%=path%>/manage/product.do?method=deleteProductInfo&productInfoIds="+productInfoId;
			document.<%=pagination!=null?pagination.getFormName():""%>.submit();
			}
	...
	<BODY> 
				...
				<TR>
					<TD class=card_blank_73 width=10>
						...
						<TD class=card_down_73 style="WIDTH: 140px">
						 ...
						 <FONT id=beChangedColor color=#000000>审核未通过(<%=unAuditCount %>)</FONT>
						</TD>
						<%
						}else
						{
						...
						<TD class=card_normal_73 style="WIDTH: 140px">
						   ...
						   <A href="<%=path%>/manage/product.do?method=productInfoList&auditType=<%=AppConstants.CPGQXX_SFYX_3 %>&overdue=<%=AppConstants.CPGOXX_UN_OVERDUE %>"> <FONT id=beChangedColor color=#000000>审核未通过(<%=unAuditCount %>)</FONT></A>
					   ...
					   %>
						...
						<TD class=card_down_73 style="WIDTH: 140px">
						 ...
						 <FONT id=beChangedColor color=#000000>已发布上网(<%=inNetCount %>)</FONT>
						</TD>
						<%
						}else
						{
						...
						<A href="<%=path%>/manage/product.do?method=productInfoList&auditType=<%=AppConstants.CPGQXX_SFYX_1 %>&overdue=<%=AppConstants.CPGOXX_UN_OVERDUE %>"> <FONT id=beChangedColor color=#000000>已发布上网(<%=inNetCount %>)</FONT></A>
					   ...
					   <%if(auditType.equals(AppConstants.CPGQXX_SFYX_2)&&!isOverdue){
						...
						<TD class=card_down_73 style="WIDTH: 140px">
						 ...
						 <FONT id=beChangedColor color=#000000>审核中(<%=inAuditCount %>)</FONT>
						</TD>
						<%
						}else
						{
						...
						<A href="<%=path%>/manage/product.do?method=productInfoList&auditType=<%=AppConstants.CPGQXX_SFYX_2 %>&overdue=<%=AppConstants.CPGOXX_UN_OVERDUE %>"> <FONT id=beChangedColor color=#000000>审核中(<%=inAuditCount %>)</FONT></A>
					 ...
					 <%if(isOverdue){
						...
						<TD class=card_down_73 style="WIDTH: 140px">
						 ...
						 <FONT id=beChangedColor color=#000000>已过期(<%=overCount %>)</FONT>
						</TD>
						<%
						}else
						{
						...
						<A href="<%=path%>/manage/product.do?method=productInfoList&overdue=<%=AppConstants.CPGOXX_OVERDUE %>"> <FONT id=beChangedColor color=#000000>已过期(<%=overCount %>)</FONT></A>
		...
		<FORM style="MARGIN: 0px" name=manageSearchForm method=post action="<%=path%>/manage/product.do?method=productInfoList&auditType=<%=auditType %>&overdue=<%=AppConstants.CPGOXX_UN_OVERDUE %>"> 
					...
					<TD class=list_lb_content align=left>
						...
						<DIV style="FLOAT: left">
							<A
								...
								href="<%=path %>/manage/product.do?method=showProductInfo&productInfoId=<%=id %>"
