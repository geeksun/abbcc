							...
							<table width='100%'>
								<tr>
							 	...
							 	if(product!=null){
							 		...
							 		if(productType!=Product.PRODUCT_TYPE_NO_PRODUCT){
							...
							%> 
							 ...
							 <tr> 
							 	 <td align='center'  > 
							 		  <%=count %></td> 
						         <td align='center'   > 
						            <textarea id='property<%=count %>' name='property'  size='17'  ><%=propertyNames[i] %></textarea> 
						             <font color='#ff0000'>*</font> 
							     </td> 
							     <td align='center'  > 
								 ...
								 <td align='center'   > 
									  <select id='select<%=count %>' disabled   > 
							    		  ...
							    		  <option value='select'  <%=types[i].equals("select")?" selected ":"" %>   >  下拉 </option> 
							    		   <option value='checkbox'  <%=types[i].equals("checkbox")?" selected ":"" %>   >  多选框 </option> 
									  </select> 
	...
	<script language="JavaScript" type="text/javascript">
		...
		function checkSumbit(){
			...
			if(checkValue())
			{
				...
				document.form_product.action="<%=path%>/manage/product.do?method=updateProduct";
