 ...
 <HTML>
	<HEAD>
		...
		<script type="text/javascript">
			...
			var action=document.<%=pagination!=null?pagination.getFormName():""%>.action="<%=path%>/productInfo.do?method=deleteProductInfo&productInfoIds="+productInfoId;
	...
	<BODY> 
				...
				<TR>
					<TD class=card_blank_73 width=10>
						...
						<TD class=card_normal_73 style="WIDTH: 140px">
						   ...
						   <A href="<%=path%>/productInfo.do?method=productInfoList&auditType=<%=AppConstants.CPGQXX_SFYX_3 %>&overdue=<%=AppConstants.CPGOXX_UN_OVERDUE %>"> <FONT id=beChangedColor color=#000000>审核未通过(<%=unAuditCount %>)</FONT></A>
					   ...
					   %>
						...
						<A href="<%=path%>/productInfo.do?method=productInfoList&auditType=<%=AppConstants.CPGQXX_SFYX_1 %>&overdue=<%=AppConstants.CPGOXX_UN_OVERDUE %>"> <FONT id=beChangedColor color=#000000>已发布上网(<%=inNetCount %>)</FONT></A>
					   ...
					   <%if(auditType.equals(AppConstants.CPGQXX_SFYX_2)&&!isOverdue){
						...
						<A href="<%=path%>/productInfo.do?method=productInfoList&auditType=<%=AppConstants.CPGQXX_SFYX_2 %>&overdue=<%=AppConstants.CPGOXX_UN_OVERDUE %>"> <FONT id=beChangedColor color=#000000>审核中(<%=inAuditCount %>)</FONT></A>
					 ...
					 <%if(isOverdue){
						...
						<A href="<%=path%>/productInfo.do?method=productInfoList&overdue=<%=AppConstants.CPGOXX_OVERDUE %>"> <FONT id=beChangedColor color=#000000>已过期(<%=overCount %>)</FONT></A>
		...
		<FORM style="MARGIN: 0px" name=manageSearchForm method=post action="<%=path%>/productInfo.do?method=productInfoList&auditType=<%=auditType %>&overdue=<%=AppConstants.CPGOXX_UN_OVERDUE %>"> 
					...
					<TD class=list_lb_content align=left>
						...
						<DIV style="FLOAT: left">
							<A
								...
								href="<%=path %>/productInfo.do?method=editProductInfo&productInfoId=<%=id %>"
