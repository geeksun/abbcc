...
public class ProductInfoAction extends BaseAction {
	...
	public ActionForward productInfoList(ActionMapping mapping,
		...
		try {
			...
			long inAuditCount=this.productService.getProductCountByType(_userId,AppConstants.PRODUCT_TYPE_IN_AUDIT);
			long inNetCount=this.productService.getProductCountByType(_userId,AppConstants.PRODUCT_TYPE_IN_NET);
			long unAuditCount=this.productService.getProductCountByType(_userId,AppConstants.PRODUCT_TYPE_UN_AUDIT);
			long overCount=this.productService.getProductCountByType(_userId,AppConstants.PRODUCT_TYPE_OVERDUE);
	...
	public ActionForward showCategoryProductInfo(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {

		int parentid = 0;
		
		int productTypeParentId=RequestUtils.getIntParameter(request, "productTypeId", -1);
		...
		try {
				...
				List<ProductType> secondCategory = this.productService.getProductTypeByParentId(productTypeParentId);
				request.setAttribute("secondCategory", secondCategory);
				if (secondCategory != null && secondCategory.size() > 0) {
					ProductType secondProductType = secondCategory.get(0);
					List<ProductType> thirdCategory = this.productService
	...
	public ActionForward editProductInfo (ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		try {
			...
			String productInfoId = RequestUtils.getParameter(request,
					"productInfoId");
			//if(productInfoId==null)return mapping.findForward("showProductInfo");
	...
	public ActionForward updateProductInfo(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			long productInfoId = RequestUtils.getLongParameter(request,
					"productInfoId", -1);// 产品id
			if (productInfoId == -1) {
				return mapping.findForward("error");
			}
			...
			FormFile updatePic2 = theForm.getUpdatePic2(); // 上传图片2
			FormFile updatePic3 = theForm.getUpdatePic3();// 上传图片3
			String updatePic1Name = UploadUtil.getGUIDFileName(updatePic1,
					request);
			String updatePic2Name = UploadUtil.getGUIDFileName(updatePic2,
					request);
			String updatePic3Name = UploadUtil.getGUIDFileName(updatePic3,
					request);
			...
			if (hasNull || !isMathValues) {
				...
				if (productInfo != null) {
					...
					if (product != null) {
						...
						jytj.setZxqdl(orderCount);
						this.productService
								.updateProductInfo(obj, cpgqxx, jytj);

						if (uploadPicState1.equals("upload")) {
							String oldPicName = cpgqxx.getTp1();
							if (oldPicName != null) {
								UploadUtil
										.deleteProductPic(oldPicName, request);
							}
							UploadUtil.saveProductPic(updatePic1,
									updatePic1Name, request);
						...
						if (uploadPicState2.equals("upload")) {
							...
							UploadUtil.saveProductPic(updatePic2,
									updatePic2Name, request);
						} else if (uploadPicState1.equals("delete")) {
							String oldPicName = cpgqxx.getTp2();
							if (oldPicName != null) {
								UploadUtil
						...
						if (uploadPicState1.equals("upload")) {
							...
							UploadUtil.saveProductPic(updatePic3,
									updatePic3Name, request);
						} else if (uploadPicState1.equals("delete")) {
							String oldPicName = cpgqxx.getTp3();
							if (oldPicName != null) {
								UploadUtil
						...
						return this.showProductInfo(mapping, form, request,
								response);
